***<h2>🚀Portfolio 3.0</h2>***
A high-performance, single-page developer portfolio built with a "Terminal & Tech" aesthetic. This project showcases my journey as a Data Science Student and Frontend Developer Intern, sitting at the intersection of intelligent systems and clean user interfaces.

**<h3>🛠️Tech Stack & Architecture</h3>**

This portfolio was engineered to be lightweight and blazing fast, prioritizing pure code over heavy frameworks.

+ **Core:** Modern HTML5 & CSS3

+ **Interactivity:** Vanilla JavaScript for scroll-reveal and terminal animations

+ **Styling:** 
    - CSS Grid & Flexbox for a fully responsive, matrix-style layout, 
    - Glassmorphism & Neon UI for a futuristic visual feel
    - Custom Keyframe Animations for staggered entry effects

+ **Typography:** Space Mono (Tech-feel) & DM Sans (Modern readability)

**<h3>✨Key Features</h3>**
+ **Responsive Matrix Design:** A mobile-first layout that adapts seamlessly from ultra-wide monitors to smartphones.

+ **Interactive Project Matrix:** Custom-built cards featuring a "macOS/Terminal" header style, complete with tech badges and live links.

+ **Dynamic Tech Stack:** Categorized skills ranging from Machine Learning (Python, scikit-learn) to Frontend Dev (React, JS).

+ **Academic Timeline:** A vertical, CSS-only journey path highlighting B.Tech progress and NPTEL certifications.

+ **Glassmorphism Navigation:** A frosted-glass header with an active blinking terminal cursor.

**<h3>📁Highlighted Projects</h3>**

The portfolio acts as a central hub for my technical work, including:

**🌐AI-Powered Plagiarism Detector:** A modern Machine Learning web application that scans text and checks for plagiarism against the live web in real time. Built entirely with Python and hosted serverless on the cloud.

**📉Customer Churn Prediction:** A Machine Learning project that predicts whether a telecom customer is likely to churn (cancel their subscription), based on their account and usage profile. Includes an interactive Streamlit dashboard for live predictions.

**Mubs_Cine:** A movie booking UI demonstrating SubtleCrypto API for secure authentication.


